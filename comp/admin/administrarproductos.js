angular
.module('adminprodApp',[])
.controller('adminprodCtrl', function($scope,$http,$location){
	var peticion = {
					"url" : "php/comunes/comprobarSesion.php",
					"method" : "POST"
					}
	$http(peticion).then(function(resp){
		if (resp.data.resp='OK') {
			if (resp.data.permisos!='1') {
				$location.path('/home');
			}}
	},function(fail){
		alert("ERROR");
	});


	$scope.miscategorias='todas';
	$scope.mistiendas='todas';
	
	$scope.mostrarproductos = function(){
		var peticion = {
		"url" : "php/admin/obtenerProductos.php",
		"method" : "POST",
		"data" : {categoria: $scope.miscategorias,tienda: $scope.mistiendas}};
		waitingDialog.show('Cargando...');	
		$http(peticion).then(function(resp){
			waitingDialog.hide();
			for (var i = 0; i < resp.data.productos.length; i++) {
				resp.data.productos[i].fecha = new Date(resp.data.productos[i].fecha);
			}
			$scope.tiendas = resp.data.tiendas;
			$scope.productos = resp.data.productos;
			$scope.categorias = resp.data.categorias;
			$scope.productosexistentes = resp.data.productosexistentes;
			$scope.estados = resp.data.estados;
			$scope.cantidad = $scope.productos.length;

		},function(fail){
			alert("ERROR");
		});
	}

	$scope.mostrarproductos();

	$scope.datos = {
			id:'',rutas:'',nombre:'',stock:'',precio:'',categoria:'',fecha:'',tienda:'',descripcion:'',estado:''
		};
		
	$scope.modifica = function() {
		$scope.disabledTienda =true;
		$scope.miThis = this;
		(['id','rutas','nombre','stock','precio','categoria','tienda','descripcion','estado']).forEach(function(valor,indice){
            $scope.datos[valor]=$scope.miThis.producto[valor];
		});
		$scope.datos.fecha = new Date($scope.miThis.producto.fecha);

		$scope.modal = {
			'opcion' : ' Editar producto',
			'boton' : 'Guardar cambios',
			'bcolor' : 'bg-primary',
			'pcolor' : 'panel-default',
			'cboton' : 'btn-primary',
			'id' : this.producto.id,
			'datos' : $scope.datos,
			'fecha' : ('').hoy()
		};
		$('#mRef').modal('show');
	}


	$scope.borrar = function() {
		$scope.miThis = this;
		(['id','rutas','nombre','stock','precio','categoria','tienda','descripcion','estado']).forEach(function(valor,indice){
            $scope.datos[valor]=$scope.miThis.producto[valor];
		});
		$scope.datos.fecha = new Date($scope.miThis.producto.fecha);

		$scope.modal = {
			'opcion' : ' Borrar producto',
			'boton' : 'Borrar producto',
			'bcolor' : 'bg-danger',
			'pcolor' : 'panel-default',
			'cboton' : 'btn-danger',
			'id' : this.producto.id,
			'datos' : $scope.datos,
			'fecha' : ('').hoy()
		};
		$('#mRef').modal({
			show:true,
			background:false,
			keyboard:false});
	}

	$scope.agregar = function() {
		$scope.datos = {
			id:'',rutas:'',nombre:'',stock:'',precio:'',categoria:'',fecha:'',tienda:'',descripcion:'',estado:''
		};
		$scope.modal = {
			'opcion' : ' Agregar producto',
			'boton' : 'Agregar',
			'bcolor' : 'bg-success',
			'pcolor' : 'panel-default',
			'cboton' : 'btn-success',
			'id' : $scope.datos.id,
			'datos' : $scope.datos,
			'fecha' : ('').hoy()
		};
		$('#mRef').modal({
			show:true,
			background:false,
			keyboard:false});
	}

	$scope.modificaImg = function () {
		$scope.miThis = this;
		(['id','rutas','nombre','stock','precio','categoria','tienda','descripcion','estado']).forEach(function(valor,indice){
            $scope.datos[valor]=$scope.miThis.producto[valor];
		});
		$scope.datos.fecha = new Date($scope.miThis.producto.fecha);
		$scope.modal = {
			'opcion' : ' Editar imágenes',
			'boton' : 'Subir',
			'bcolor' : 'bg-info',
			'pcolor' : 'panel-info',
			'cboton' : 'btn-success',
			'id' : '',
			'datos' : $scope.datos,
			'fecha' : ('').hoy()
		};
		$('#mRef2').modal({
			show:true,
			background:false,
			keyboard:false});
	}

	$scope.modificaEstado = function () {
		$scope.miThis = this;
		(['id','rutas','nombre','stock','precio','categoria','tienda','descripcion','estado']).forEach(function(valor,indice){
            $scope.datos[valor]=$scope.miThis.producto[valor];
		});
		$scope.datos.fecha = new Date($scope.miThis.producto.fecha);
		$scope.modal = {
			'opcion' : ' Cambiar estado',
			'boton' : 'Guardar estado',
			'bcolor' : 'bg-warning',
			'pcolor' : 'panel-default',
			'cboton' : 'btn-success',
			'id' : '',
			'datos' : $scope.datos,
			'fecha' : ('').hoy()
		};
		$('#mRef3').modal({
			show:true,
			background:false,
			keyboard:false});
	}

	$scope.aEliminar =[];
	$scope.agregarAeliminar = function(){
		$scope.aEliminar.push(this.ruta);
	}

	$scope.eliminarImagenes = function(){
		var peticion = {
						"url" : "php/admin/eliminarImagenes.php",
						"method" : "POST",
						"data" : {
								idproducto:this.datos.id,
								rutas: $scope.aEliminar
							}};			
				$http(peticion).then(function(resp){
					$scope.aEliminar=[];
					alert("Imagenes eliminadas correctamente");
					$scope.mostrarproductos();
				},function(fail){
					alert("ERROR");
				});
	}

	$scope.accion = function(){
		switch ($scope.modal.boton) {
			case 'Guardar cambios':
				(['id','rutas','nombre','stock','precio','categoria','tienda','descripcion','estado']).forEach(function(valor,indice){
		            $scope.miThis.producto[valor]=$scope.datos[valor];
				});
				$scope.miThis.producto.fecha = new Date($scope.datos.fecha);

				var peticion = {
						"url" : "php/admin/modificarProducto.php",
						"method" : "POST",
						"data" : {
								producto: $scope.miThis.producto
							}};			
				$http(peticion).then(function(resp){
					$('#mRef').modal('toggle');
					$scope.mostrarproductos();
				},function(fail){
					alert("ERROR");
				});
				break;
			case 'Guardar estado':
				var peticion = {
						"url" : "php/admin/modificarEstadoProducto.php",
						"method" : "POST",
						"data" : {
								estado: $scope.nuevoEstado,
								producto: $scope.modal.datos.id
							}};			
				$http(peticion).then(function(resp){
					$('#mRef3').modal('toggle');
					$scope.mostrarproductos();
				},function(fail){
					alert("ERROR");
				});
				$('#mRef4').modal('toggle');
				break;
			case 'Subir':
					var file = $scope.file;
					var myID = $scope.modal.datos.id;
					var formData = new FormData();
					formData.append('folder',myID);
					for (var i = 0; i < file.length; i++) {
						formData.append(i, file[i]);
					}
					return $http.post('php/admin/subirImagenes.php',formData, {
						headers: {
							'Content-type': undefined
						},
						transformRequest: angular.identity
					}).then(function(resp){
						$scope.mostrarproductos();
					},function(fail){
						alert("ERROR");
					});
				break;
			default:
				break;
		}
	}
})
.filter('euros', function() {
return function (texto,moneda) {
	return parseFloat(texto).formato(moneda);
	}
})
.filter('vfecha', function() {
	return function vfecha(texto) {
		return (texto)?texto.fv():'';
	}
})
.directive('subirImagen', function($parse){
	return {
		restrict:'A',
		link: function(scope,iElement,iAttrs){
			iElement.on("change",function(e){
				$parse(iAttrs.subirImagen).assign(scope,iElement[0].files);
			});
		}
	}
});

